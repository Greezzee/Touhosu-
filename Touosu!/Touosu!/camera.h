...
private:
	...
	bool action(int i, bool new_tick, float player_x, float player_y, RenderWindow *window, float time) {
		if (active_actions[i].type == "rotate") {
			...
			float rotate;
			if (active_actions[i].isRotateClockwise) {
				if (abs(cam_angle - active_actions[i].endAngle) <= 180) rotate = -abs(cam_angle - active_actions[i].endAngle);
				else rotate = -360 + abs(cam_angle - active_actions[i].endAngle);
			}
			else {
				if (abs(cam_angle - active_actions[i].endAngle) <= 180) rotate = abs(cam_angle - active_actions[i].endAngle);
				else rotate = 360 - abs(cam_angle - active_actions[i].endAngle);
			...
			if (active_actions[i].endBeat - current_beat <= 32) {
